...
class ScoringServiceTest {
    ...
    private void setupMocks() {
        ...
        for (int i = 1; i <= 10; i++) {
            when(questionService.findById(i)).thenReturn(Optional.of(makeQuestion(i, "M")));
        }
        // Questions 11-20: C axis
        for (int i = 11; i <= 20; i++) {
            when(questionService.findById(i)).thenReturn(Optional.of(makeQuestion(i, "C")));
        }
        // Questions 21-30: R axis
        for (int i = 21; i <= 30; i++) {
            when(questionService.findById(i)).thenReturn(Optional.of(makeQuestion(i, "R")));
        }
    }
    ...
    void testInvalidValue() {
        when(questionService.findById(1)).thenReturn(Optional.of(makeQuestion(1, "M")));
    ...
    void testPositionBounds() {
        ...
        for (int[] pattern : patterns) {
            ...
            assertTrue(result.getX() >= 0 && result.getX() <= 1,
                "X out of bounds: " + result.getX());
            
            // Y should be between 0 and ~0.866
            assertTrue(result.getY() >= 0 && result.getY() <= 0.867,
