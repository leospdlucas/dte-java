...
class QuizControllerTest {
  ...
  void testGetQuestions() throws Exception {
    // Setup
    when(questionService.getBalancedQuestions()).thenReturn(
      Arrays.asList(
  ...
  void testGetLatestResult() throws Exception {
    ...
    when(submissionService.getLatestForUser(123L)).thenReturn(
      Optional.of(result)
    );

    mockMvc
      .perform(get("/api/my-latest").header("X-User-Id", "123"))
      .andExpect(status().isOk())
  ...
  void testNoSubmissions() throws Exception {
    when(submissionService.getLatestForUser(anyLong())).thenReturn(
      Optional.empty()
    );
