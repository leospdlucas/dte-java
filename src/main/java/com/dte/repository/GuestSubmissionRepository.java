...
@Repository
public interface GuestSubmissionRepository
  extends JpaRepository<GuestSubmission, Long>
{
  ...
  // Get average scores
  @Query(
    "SELECT AVG(g.scoreM), AVG(g.scoreC), AVG(g.scoreR) FROM GuestSubmission g"
  )
  ...
  @Query(
    "SELECT g.coordX, g.coordY FROM GuestSubmission g WHERE g.inviterUserId = :userId"
  )
  List<Object[]> findCoordinatesByInviter(@Param("userId") Long userId);

  // Get average scores by inviter
  @Query(
    "SELECT AVG(g.scoreM), AVG(g.scoreC), AVG(g.scoreR) FROM GuestSubmission g WHERE g.inviterUserId = :userId"
  )
